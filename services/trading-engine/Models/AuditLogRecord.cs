...
public sealed class AuditLogRecord
{
    ...
    public static AuditLogRecord RiskSettingsUpdated(RiskSettings settings)
    {
        return new AuditLogRecord
        {
            ...
            Details = $"Risk settings updated: auto={settings.EnableAutoTrading}, requireApproval={settings.RequireSignalApproval}, testMode={settings.TestMode}, ignoreTp2={settings.IgnoreTakeProfit2}, locked={settings.TradingLocked}, emergency={settings.EmergencyStopActive}, maxContracts={settings.MaxContractsPerSignal}, maxLossTrade={settings.MaxLossPerTrade}, maxDailyLoss={settings.MaxDailyLoss}, tradingDayTimeZone={settings.TradingDayTimeZoneId}, maxEntryDistance={settings.MaxEntryPriceDeviationPoints}, chartMarketProtectionDistance={settings.ChartMarketProtectionDistancePoints}, symbols={string.Join(",", settings.AllowedSymbols)}",
            CreatedAt = DateTimeOffset.UtcNow
        };
    }
