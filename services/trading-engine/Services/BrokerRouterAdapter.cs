    ...
    IBKRBrokerAdapter ibkrBrokerAdapter) : IBrokerAdapter
{
    ...
    public Task<MarketOrderResult> PlaceMarketOrderAsync(string symbol, string direction, int contracts, decimal? referencePrice, TradingDbContext db, bool attachProtection = false, decimal? protectionDistance = null, decimal? stopLoss = null, decimal? takeProfit = null)
    {
        return ActiveAdapter.PlaceMarketOrderAsync(symbol, direction, contracts, referencePrice, db, attachProtection, protectionDistance, stopLoss, takeProfit);
    }
